            ...
            <React.Fragment>
                ...
                <Grid container xs={12} style={{ top: 10, alignItems: "center", justifyContent: "center" }}>
                    ...
                    <Paper sx={{ p: 3, mb: 4, backgroundColor: theme.palette.mode === 'dark' ? "#3a3b3c" : "whitesmoke", }} style={{ display: "flex", flexDirection: "column", alignItems: "space-between" }}
                        variant="outlined">
                        <Typography align="center" sx={{ m: 2 }}> <LightbulbCircleIcon style={{ color: "red" }} /> Remeber any interview amenitys? Please do share! </Typography>
                        {
                            amenityList.map((singleamenity, index) => (
                                <Grid>
                                    {(amenityList.length == 1) ?
                                        (
                                            <Grid style={{ display: "flex", flexDirection: "row" }}>
                                                <TextField name={"amenity"} value={singleamenity.amenity} onChange={(event) => handleamenityChange(event, index)} style={{ marginBottom: 10, marginRight: 5 }} label={"Enter new interview amenity"} fullWidth />
                                            </Grid>
                                        ) :
                                        (
                                            <Grid style={{ display: "flex", flexDirection: "row" }}>
